...
export class Connector {
  ...
  returnSuccess (response) {
    return (response.error) ? this.returnError(response) : response
  }
  ...
  async register (username, password, name, email, code) {
    try {
      const resp = await this.post(`${this.url}/api`, {
        request: 'register',
        username: username,
        password: password,
        name: name,
   ...
   * @param {String} giftId
   */
  async addBuyer (giftId, giftState) {
    try {
      const resp = await this.post(`${this.url}/api`, {
        token: this.token,
        request: 'add_buyer',
  ...
  /**
   * Update buyer status for a gift.
   * @param {String} giftId
   */
  async updateBuyer (giftId, giftState) {
    try {
      const resp = await this.post(`${this.url}/api`, {
        token: this.token,
        request: 'update_buyer',
        buyerId: giftId,
        giftState: giftState
      })
      return this.returnSuccess(resp)
